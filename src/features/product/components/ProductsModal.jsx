...
export const ProductModal = ({ isOpen, onClose, product }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (product) {
        ...
        reset({ nombre: "", descripcion: "", precio: "", categoria: "" });
      }
    }
  ...
  const onSubmit = async (data) => {
    try {
      ...
      showSuccess(product ? "Producto actualizado correctamente" : "Producto creado correctamente");
      reset();
      onClose();
    } catch (error) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-4 sm:p-6 space-y-5 overflow-y-auto">
          ...
          <div className="flex gap-3 pt-4 border-t justify-end">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2 bg-red-600 hover:bg-red-700 text-white rounded transition-colors flex items-center justify-center min-w-[120px]"
            >
              {loading ? <Spinner className="h-4 w-4" /> : product ? "Guardar cambios" : "Crear producto"}
            </button>
          </div>
        </form>
      </div>
