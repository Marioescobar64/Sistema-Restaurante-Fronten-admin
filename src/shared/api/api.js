...
axiosAdmin.interceptors.request.use((config) => {
    ...
    // Solo aplicar JSON si el body NO es FormData
    if (config.data && !(config.data instanceof FormData)) {
        config.headers["Content-Type"] = "application/json";

        // Eliminar campos que sean objetos vacíos, FileList vacío, o null
        if (typeof config.data === "object") {
            const cleaned = {};
            for (const [key, value] of Object.entries(config.data)) {
                // Saltar FileList vacío, objeto vacío {}, null y undefined
                if (value === null || value === undefined) continue;
                if (value instanceof FileList && value.length === 0) continue;
                if (
                    typeof value === "object" &&
